...
vtkStandardNewMacro(f3d_assimp_importer);

class f3d_assimp_importer::vtkInternals
{
public:
  //----------------------------------------------------------------------------
  explicit vtkInternals(f3d_assimp_importer* parent)
    : Parent(parent)
  {
  }
  ...
  f3d_assimp_importer* Parent;
};

//----------------------------------------------------------------------------
f3d_assimp_importer::f3d_assimp_importer()
  : Internals(new f3d_assimp_importer::vtkInternals(this))
{
}

//----------------------------------------------------------------------------
f3d_assimp_importer::~f3d_assimp_importer() = default;

//----------------------------------------------------------------------------
int f3d_assimp_importer::ImportBegin()
{
  return this->Internals->ReadScene(this->FileName);
}

//----------------------------------------------------------------------------
void f3d_assimp_importer::ImportActors(vtkRenderer* renderer)
{
...
//----------------------------------------------------------------------------
std::string f3d_assimp_importer::GetOutputsDescription()
{
  return this->Internals->Description;
}

//----------------------------------------------------------------------------
bool f3d_assimp_importer::UpdateAtTimeValue(double timeValue)
{
...
//----------------------------------------------------------------------------
vtkIdType f3d_assimp_importer::GetNumberOfAnimations()
{
  return this->Internals->Scene ? this->Internals->Scene->mNumAnimations : 0;
}

//----------------------------------------------------------------------------
std::string f3d_assimp_importer::GetAnimationName(vtkIdType animationIndex)
{
...
//----------------------------------------------------------------------------
void f3d_assimp_importer::EnableAnimation(vtkIdType animationIndex)
{
  assert(animationIndex < this->GetNumberOfAnimations());
  assert(animationIndex >= 0);
...
//----------------------------------------------------------------------------
void f3d_assimp_importer::DisableAnimation(vtkIdType vtkNotUsed(animationIndex))
{
  this->Internals->ActiveAnimation = -1;
}

//----------------------------------------------------------------------------
bool f3d_assimp_importer::IsAnimationEnabled(vtkIdType animationIndex)
{
  assert(animationIndex < this->GetNumberOfAnimations());
  assert(animationIndex >= 0);
...
//----------------------------------------------------------------------------
bool f3d_assimp_importer::GetTemporalInformation(vtkIdType animationIndex,
...
//----------------------------------------------------------------------------
vtkIdType f3d_assimp_importer::GetNumberOfCameras()
{
  return this->Internals->Cameras.size();
}

//----------------------------------------------------------------------------
std::string f3d_assimp_importer::GetCameraName(vtkIdType camIndex)
{
  return this->Internals->Cameras[camIndex].first;
}

//----------------------------------------------------------------------------
void f3d_assimp_importer::SetCamera(vtkIdType camIndex)
{
  this->Internals->ActiveCameraIndex = camIndex;
}

//----------------------------------------------------------------------------
void f3d_assimp_importer::ImportCameras(vtkRenderer* renderer)
{
  this->Internals->ImportCameras(renderer);
}

//----------------------------------------------------------------------------
void f3d_assimp_importer::ImportLights(vtkRenderer* renderer)
{
